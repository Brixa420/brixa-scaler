...
func main() {
	...
	urls := make([]string, 10)
	for i := 0; i < 10; i++ {
		urls[i] = fmt.Sprintf("http://localhost:%d/prove", 3111+i)
	}
	zk := NewMultiZKProver(urls)

	fmt.Println("╔════════════════════════════════════════════════════════════════╗")
	fmt.Println("║        FULL PIPELINE: 10 GNARK SERVICES (No HTTP Overhead)    ║")
	fmt.Println("╚════════════════════════════════════════════════════════════════╝")

	// Test single proof latency
	batch := createBatchSharded(0, 1000, 10)
	proof, _ := zk.Prove(batch)
	fmt.Printf("\nSingle proof latency: %dms\n", proof.ProveMs)
	...
	// Full pipeline with 200 provers + 10 gnark
	fmt.Println("\n--- Layer 2: ZK (200 provers → 10 gnark services) ---")
	...
	fmt.Println("\n✅ 10 gnark services = ~13K TPS theoretical max!")
}
