...
func NewHTTPProver() *HTTPProver {
	return &HTTPProver{url: "http://localhost:3111/prove"}
}
...
func (h *HTTPProver) Prove(batch *Batch) (*Proof, error) {
	...
	resp, err := http.Post(h.url, "application/json", bytes.NewReader(jsonData))
	if err != nil {
...
func main() {
	...
	fmt.Println("╔════════════════════════════════════════════════════════════════╗")
	fmt.Println("║     REAL ZK PIPELINE: Go Batching → HTTP ZK Service         ║")
	fmt.Println("╚════════════════════════════════════════════════════════════════╝")
	
	// Test single
	batch := createBatchSharded(0, 1000, 10)
	proof, err := httpProver.Prove(batch)
	if err != nil {
		fmt.Println("Error:", err)
	} else {
		fmt.Printf("\n--- Single Real ZK: %dms, Valid: %v ---\n", proof.ProveMs, proof.Valid)
	}
	...
	// Real ZK via HTTP
	fmt.Println("\n--- Layer 2: REAL ZK via HTTP ---")
	pool := NewProverPool(50, httpProver)
	pool.Start()
