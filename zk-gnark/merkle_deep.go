...
// Custom JSON marshaler for big.Int to ensure proper string output
...
// 8-level Merkle tree (256 leaves)
...
func (c *MerkleCircuit) Define(api frontend.API) error {
	...
	api.AssertIsEqual(c.Root, current7)
	return nil
}
...
func main() {
	...
	fmt.Println("╔══════════════════════════════════════════════════════════════╗")
	fmt.Println("║     8-LEVEL MERKLE (256 leaves) + POSEIDON HASH            ║")
	fmt.Println("╚══════════════════════════════════════════════════════════════╝")
	...
	http.HandleFunc("/buildtree", func(w http.ResponseWriter, r *http.Request) {
		...
		for i := range leaves {
			...
			// Convert siblings to strings explicitly
	...
	http.HandleFunc("/prove", func(w http.ResponseWriter, r *http.Request) {
		...
		for i := 0; i < 8 && i < len(req.Siblings); i++ {
			p := new(big.Int)
			if req.Siblings[i] != "" {
