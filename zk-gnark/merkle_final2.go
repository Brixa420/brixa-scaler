...
func main() {
	...
	fmt.Println("╔══════════════════════════════════════════════════════════════╗")
	fmt.Println("║         MERKLE CIRCUIT - Debug Mode                         ║")
	fmt.Println("╚══════════════════════════════════════════════════════════════╝")
	...
	http.HandleFunc("/prove", func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Printf("Leaf (mod): %s\n", leafVal)
		fmt.Printf("Root (mod): %s\n", rootVal)
		...
		// Debug: Compute expected hash
		current := leafVal
		if req.Bits[0] == 0 {
			current = hashGo(current, sibVals[0])
		} else {
			current = hashGo(sibVals[0], current)
		}
		if req.Bits[1] == 0 {
			current = hashGo(current, sibVals[1])
		} else {
			current = hashGo(sibVals[1], current)
		}
		if req.Bits[2] == 0 {
			current = hashGo(current, sibVals[2])
		} else {
			current = hashGo(sibVals[2], current)
		}
		if req.Bits[3] == 0 {
			current = hashGo(current, sibVals[3])
		} else {
			current = hashGo(sibVals[3], current)
		}
		fmt.Printf("Expected final: %s\n", current)
		...
		fmt.Printf("Proving done: %dms, err: %v\n", proveMs, err)
		...
		fmt.Printf("Verify done: %dms, valid: %v, err: %v\n", verifyMs, err == nil, err)
