...
// MerkleCircuit - CORRECT hash computation
type MerkleCircuit struct {
	Root  frontend.Variable `gnark:",public"`
	Leaf frontend.Variable  `gnark:"secret"`
	
	// Sibling nodes at each level
...
func (c *MerkleCircuit) Define(api frontend.API) error {
	// Level 0: if bit0=0, hash(leaf, sibling0). if bit0=1, hash(sibling0, leaf)
...
// Hash function in Go (same as circuit)
