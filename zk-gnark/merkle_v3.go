...
// MerkleCircuit - compute both hash orders, select based on bit
...
func (c *MerkleCircuit) Define(api frontend.API) error {
	// Level 0: compute both orders, select
	h0a := hash(api, c.Leaf, c.Path0) // leaf || path0
	h0b := hash(api, c.Path0, c.Leaf) // path0 || leaf
	current0 := api.Select(c.Bit0, h0b, h0a)
	
	// Level 1
	h1a := hash(api, current0, c.Path1)
	h1b := hash(api, c.Path1, current0)
	current1 := api.Select(c.Bit1, h1b, h1a)
	
	// Level 2
	h2a := hash(api, current1, c.Path2)
	h2b := hash(api, c.Path2, current1)
	current2 := api.Select(c.Bit2, h2b, h2a)
	
	// Level 3 (root)
	h3a := hash(api, current2, c.Path3)
	h3b := hash(api, c.Path3, current2)
...
func main() {
	...
	fmt.Println("╔══════════════════════════════════════════════════════════════╗")
	fmt.Println("║         MERKLE CIRCUIT V3 - Fixed Branch Selection         ║")
	fmt.Println("╚══════════════════════════════════════════════════════════════╝")

	r1csCompiled, _ = frontend.Compile(fieldMod, r1cs.NewBuilder, &MerkleCircuit{})
	fmt.Println("Circuit: 4-level Merkle, 9 private inputs")
