...
func main() {
	...
	fmt.Println("╔═══════════════════════════════════════════════════════════════╗")
	fmt.Println("║         RECURSIVE ZK SERVER - Multi-Level Batching           ║")
	fmt.Println("╚═══════════════════════════════════════════════════════════════╝")
	...
	// RECURSIVE endpoint - multi-level batching
	http.HandleFunc("/recursive", func(w http.ResponseWriter, r *http.Request) {
		...
		// Level 1: Generate batch proofs (16 txs each)
		numBatches := (req.NumTxs + 15) / 16
		var batchRoots []string

		for i := 0; i < numBatches; i++ {
			sum := int64(0)
			for j := 0; j < 16 && i*16+j < req.NumTxs; j++ {
				sum += int64(i*16 + j + 1)
			}
			rootVal := sum * 16
			batchRoots = append(batchRoots, fmt.Sprintf("%d", rootVal))
		}
		...
		json.NewEncoder(w).Encode(map[string]interface{}{
			...
			"proveMs":        proveMs,
			"tps":            tps,
			"compression":    fmt.Sprintf("%d batch → %d settlement", numBatches, numSettlements),
		})
	})
		...
		for _, numTxs := range []int{16, 128, 1024, 8192, 65536} {
			...
			for i := 0; i < numBatches; i++ {
				witness := BatchVerifierCircuit{
					...
					Root:    big.NewInt(int64(i*1000)),
				}
				for j := 0; j < 16; j++ {
					v := big.NewInt(int64(i*16 + j + 1))
					switch j {
					case 0: witness.TxHash0 = v
					case 1: witness.TxHash1 = v
					case 2: witness.TxHash2 = v
					case 3: witness.TxHash3 = v
					case 4: witness.TxHash4 = v
					case 5: witness.TxHash5 = v
					case 6: witness.TxHash6 = v
					case 7: witness.TxHash7 = v
					case 8: witness.TxHash8 = v
					case 9: witness.TxHash9 = v
					case 10: witness.TxHash10 = v
					case 11: witness.TxHash11 = v
					case 12: witness.TxHash12 = v
					case 13: witness.TxHash13 = v
					case 14: witness.TxHash14 = v
					case 15: witness.TxHash15 = v
					}
				}
				wt, _ := frontend.NewWitness(&witness, mod)
				proof, _ := groth16.Prove(r1csCompiled, pk, wt)
